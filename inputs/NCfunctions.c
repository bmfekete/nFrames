...
char *NCcompleteURLaddress(const char *url, char *fileName, NFtimeUnit timeUnit, NFtime_p time) {
    ...
    switch (timeUnit) {
        case NFtimeUnitYear:
            sprintf (fileName + i, "%04d%s", time->Year, url + i + timeUnit);
            break;
        case NFtimeUnitMonth:
            sprintf (fileName + i, "%04d-%02d%s", time->Year, time->Month, url + i + timeUnit);
            break;
        case NFtimeUnitDay:
            sprintf (fileName + i, "%04d-%02d-%02d%s", time->Year, time->Month, time->Day, url + i + timeUnit);
            break;
        case NFtimeUnitHour:
            sprintf (fileName + i, "%04d-%02d-%02d %02d%s", time->Year, time->Month, time->Day, time->Hour,
                     url + i + timeUnit);
            break;
        case NFtimeUnitMinute:
            sprintf (fileName + i, "%04d-%02d-%02d %02d:%02d%s", time->Year, time->Month, time->Day, time->Hour,
                     time->Minute, url + i + timeUnit);
            break;
        default:
            strcpy (fileName, url);
            break;
...
NFtimeLine_p NCtimeLineExpand(NCaxis_p timeAxis, NFtime_p beginTime, NFtime_p endTime, cv_converter *cvConverter,
