...
static NFvarType _NCgridVariableType(NFio_p io, NFvarHandle varHandle) {
    ...
    switch (vartype) {
        default:
            return (NFvarTypeVoid);
        case NC_CHAR:
            return (NFvarTypeByte);
        case NC_SHORT:
        case NC_INT:
            return (NFvarTypeShort);
        case NC_FLOAT:
            return (NFvarTypeFloat);
        case NC_DOUBLE:
            return (NFvarTypeDouble);
    }
    return (NFvarTypeVoid);
}
...
static NCgrid_p _NCgridRealloc(NCgrid_p ncGrid) {
    ...
    if ((ncGrid->FileNames = (char **) realloc(ncGrid->FileNames, (ncGrid->FileNum + 1) * sizeof(char *))) ==
        (char **) NULL) {
        CMmsgPrint(CMmsgSysError, "Memory allocation error in %s:%d!\n", __FILE__, __LINE__);
        goto Abort;
    }
    ...
    if ((ncGrid->Time = (NCaxis_p *) realloc(ncGrid->Time, (ncGrid->FileNum + 1) * sizeof(NCaxis_p))) ==
        (NCaxis_p *) NULL) {
        CMmsgPrint(CMmsgSysError, "Memory allocation error in %s:%d!\n", __FILE__, __LINE__);
        goto Abort;
    }
...
static NCgrid_p _NCgridCreate() {
    ...
    if (ncGrid->X != (NCaxis_p *) NULL) {
        NCaxisFree(ncGrid->X[0]);
        free(ncGrid->X[0]);
    }
    if (ncGrid->Y != (NCaxis_p *) NULL) {
        NCaxisFree(ncGrid->Y[0]);
        free(ncGrid->Y[0]);
    }
    if (ncGrid->Z != (NCaxis_p *) NULL) {
        NCaxisFree(ncGrid->Z[0]);
        free(ncGrid->Z[0]);
    }
    if (ncGrid->Time != (NCaxis_p *) NULL) {
        NCaxisFree(ncGrid->Time[0]);
        free(ncGrid->Time[0]);
    }
    return ((NCgrid_p) NULL);
}
