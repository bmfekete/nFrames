...
static CMreturn _NFmodelExecute(NFcompModel_p model, bool report, size_t timeBeat, NFtime_p *timeBegin,
                                NFtime_p *timeEnd, NFtime_p *beatBegin, NFtime_p *beatEnd) {
    char timeStr[NFtimeUnitMinute + 1], *separator;
    size_t i;
    CMreturn ret;
    NFcomponent_p component;

    for (NFtimeSetComplete(timeBegin[timeBeat], beatBegin[timeBeat], (NFtimeStep_p) (model->TimeSteps->List[timeBeat]),
                           NFintervalBegin);
            ...
            if (component->TimeStep == (NFtimeStep_p) (model->TimeSteps->List[timeBeat])) {
                ...
                switch (component->Type) {
                    case NFcompAggregate:
                        ret = _NFexecuteAggregate(component, report);
                        break;
                    case NFcompContainer:
                        ret = _NFexecuteContainer(component, report);
                        break;
                    case NFcompInput:
                        ret = _NFexecuteInput(component, report);
                        break;
                    case NFcompRegion:
                        ret = _NFexecuteRegion(component, report);
                        break;
                    default:
                        CMmsgPrint(CMmsgAppError, "Invalid component [%s] in %s:%d!\n", component->Name, __FILE__,
                                   __LINE__);
                        ret = CMfailed;
                        break;
                }
