...
const char *NFtimeUnitString(NFtimeUnit tUnit) {
    switch (tUnit) {
        case NFtimeUnitYear:
            return (NFtimeUnitYearStr);
        case NFtimeUnitMonth:
            return (NFtimeUnitMonthStr);
        case NFtimeUnitDay:
            return (NFtimeUnitDayStr);
        case NFtimeUnitHour:
            return (NFtimeUnitHourStr);
        case NFtimeUnitMinute:
            return (NFtimeUnitMinuteStr);
        default:
            break;
    }
    return ((char *) NULL);
...
bool NFtimeSet(NFtime_p timePtr, int year, int month, int day, int hour, int minute) {
    ...
    if (year == NFtimeUnset) {
        timePtr->Year = timePtr->Month = timePtr->Day = timePtr->Hour = timePtr->Minute = NFtimeUnset;
        return (timePtr);
    }
    timePtr->Year = (short) year;

    if (month == NFtimeUnset) {
...
bool NFtimeSetComplete(NFtime_p inTime, NFtime_p outTime, NFtimeStep_p timeStep, NFinterval interval) {
    ...
    if (day > _NFtimeMonthLength(year, month)) {
        day = 1;
        month = month + 1;
    }
    if (month > 12) {
...
bool NFtimeSetFromString(NFtime_p timePtr, const char *timeStr) {
    ...
    switch (strlen(timeStr)) {
        case  4:
            sscanf(timeStr, "%d", &year);
            break;
        case  7:
            sscanf(timeStr, "%d-%d", &year, &month);
            break;
        case 10:
...
bool NFtimeStepSetFromTimes(NFtimeStep_p timeStep, NFtime_p begin, NFtime_p end) {
    ...
    if ((begin->Day != NFtimeUnset) &&
        ...
        timeStep->Length =
                (end->Month - begin->Month) * _NFtimeMonthLength(end->Year, end->Month) + end->Day - begin->Day;
        return (true);
    }
