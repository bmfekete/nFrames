...
NFobject_p NFparseModDerivativeCreate(XML_Parser parser, NFobject_p parent, const char *name, const char **attr) {
    ...
    if ((inputVar = (NFnumVariable_p) NFobjListFindItemByName(((NFcompContainer_p) parent)->Variables, attrib)) ==
        (NFnumVariable_p) NULL) {
...
CMreturn NFparseModDerivativeFinalize(NFobject_p parent, NFobject_p object) {
    ...
    if (NFobjListAddItem(((NFcompContainer_p) parent)->Variables,
                         (NFobject_p) ((NFmodDerivative_p) object)->Variable) != CMsucceeded) {
