...
NFobject_p NFparseModProcessCreate(XML_Parser parser, NFobject_p parent, const char *name, const char **attr) {
    ...
    if ((module->Initialize = (NFinitializeFunc) NFpluginFunction(plugin, NFfuncInitializeStr, attrib)) ==
        (NFinitializeFunc) NULL) {
        CMmsgPrint(CMmsgUsrError, "Error loding plugin [%s] function [%s] in line %d!\n", plugin->Name, attrib,
                   XML_GetCurrentLineNumber(parser));
        goto Abort;
    }
    if ((module->Execute = (NFexecuteFunc) NFpluginFunction(plugin, NFfuncExecuteStr, attrib)) ==
        (NFexecuteFunc) NULL) {
        CMmsgPrint(CMmsgUsrError, "Error loding plugin [%s] function [%s] in line %d!\n", plugin->Name, attrib,
                   XML_GetCurrentLineNumber(parser));
        goto Abort;
    }
    if ((module->Finalize = (NFfinalizeFunc) NFpluginFunction(plugin, NFfuncFinalizeStr, attrib)) ==
        (NFfinalizeFunc) NULL) {
    ...
    CMmsgPrint(CMmsgInfo, "Parameter num: %d Variable num: %d\n", module->Context->ParameterNum,
               module->Context->VariableNum);
        ...
        if ((module->Context->Variables[i].Role == NFinput) ||
            ...
            for (j = 0; j < module->Aliases->Num; ++j) {
                if ((((NFobjAlias_p) module->Aliases->List[j])->AliasType == NFaliasVariable) &&
                    (strcmp(module->Context->Variables[i].Name, (((NFobjAlias_p) module->Aliases->List[j])->Alias)) ==
                     0))
                    break;
            }
            if (j == module->Aliases->Num) {
                CMmsgPrint(CMmsgAppError, "Process [%s] variable [%s] is not found!\n", module->Name,
                           module->Context->Variables[i].Name);
                goto Abort;
            }
        }
        else {
