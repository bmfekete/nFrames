...
NFobject_p NFparseNumParameterCreate(XML_Parser parser, NFobject_p parent, const char *name, const char **attr) {
    ...
    if ((parent->Type != NFcompModel) && (parent->Type != NFcompContainer) && (parent->Type != NFmodEquation) &&
        (parent->Type != NFmodProcess)) {
...
CMreturn NFparseNumParameterFinalize(NFobject_p parent, NFobject_p object) {
    ...
    switch (parent->Type) {
        case NFcompContainer:
            list = ((NFcompContainer_p) parent)->Parameters;
            break;
        case NFcompModel:
            list = ((NFcompModel_p) parent)->Parameters;
            break;
        case NFmodEquation:
            list = ((NFmodEquation_p) parent)->Parameters;
            break;
        case NFmodProcess:
            list = ((NFmodProcess_p) parent)->Parameters;
            break;
        default:
            CMmsgPrint(CMmsgAppError, "This shouldn't have happened in %s:%d!\n", __FILE__, __LINE__);
            return (CMfailed);
