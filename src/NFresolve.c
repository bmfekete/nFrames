...
static CMreturn _NFresolveVariable(NFobject_p object, bool report) {
    ...
    if (parent->Type == NFcompInput) {
        if ((variable->VarHandle = NFioVariableHandle(((NFcompInput_p) parent)->Domain->IO, variable->StandardName)) ==
            (NFvarHandle) NULL) {
            CMmsgPrint(CMmsgUsrError, "Missing input variable in line %d!\n", variable->XMLline);
            goto Abort;
        }
        if ((variable->VarType = NFioVariableType(((NFcompInput_p) parent)->Domain->IO, variable->VarHandle)) ==
            NFvarTypeVoid) {
    ...
    if (report)
        CMmsgPrint(CMmsgInfo, "Variable[%s] type %s\n", variable->StandardName, NFvarTypeString(variable->VarType));
    if ((variable->Data.Void = malloc(((NFcompInput_p) parent)->Domain->IO->ItemNum * variable->VarType)) ==
        (void *) NULL) {
...
static CMreturn _NFresolveInput(NFobject_p object, bool report) {
    ...
    if ((ioOpen = (NFioOpenFunc) NFpluginFunction(component->IOplugin, NFfuncOpenStr, component->IOmethod)) ==
        (NFioOpenFunc) NULL) {
        CMmsgPrint(CMmsgUsrError, "Error loading plugin [%s] function [%s] in line %d!\n", component->IOplugin->Name,
                   component->IOmethod, component->XMLline);
        goto Abort;
    }
    ...
    for (i = 0; i < component->Variables->Num; ++i) {
        ...
        if ((variable->VarHandle = NFioVariableHandle(component->Domain->IO, variable->StandardName)) ==
            (NFvarHandle) NULL) {
    ...
    for (i = 0; i < model->Components->Num; ++i) {
        ...
        switch (component->Type) {
            case NFcompAggregate:
                ret = _NFresolveAggregate(component, report);
                break;
            case NFcompContainer:
                ret = _NFresolveContainer(component, report);
                break;
            case NFcompInput:
                ret = _NFresolveInput(component, report);
                break;
            case NFcompRegion:
                ret = _NFresolveRegion(component, report);
                break;
            default:
